...
def _load_model_in_background():
    ...
    try:
        ...
        # Check required model artifact files before attempting to load
        required = [
            Path(model_dir) / 'movie_metadata.parquet',
            Path(model_dir) / 'title_to_idx.json',
        ...
        if missing:
            ...
            logger.error(f"Failed to load recommender: {_LOAD_ERROR}")
            return
...
def _start_model_loading():
    ...
    # If a previous load error exists, check whether files now exist and allow retry
    global _LOAD_ERROR
    model_dir = getattr(settings, 'MODEL_DIR', os.environ.get('MODEL_DIR', 'models'))
    if _LOAD_ERROR:
        ...
        if not all(p.exists() for p in required):
            logger.debug("Not starting model loading because previous load error exists and files still missing")
            return
        # files present now - clear the previous error and retry
        logger.info("Required model files found. Clearing previous load error and retrying model load.")
        _LOAD_ERROR = None
